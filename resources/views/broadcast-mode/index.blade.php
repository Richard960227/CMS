            ...
            <div class="card-body">
                ...
                <div class="max-h-96 overflow-y-auto">
                    <table id="params-table" class="table table-xs table-pin-rows">
                        ...
                        <tbody>
                            @foreach ($broadcastModes as $broadcastMode)
                                <tr class="hover">
                                    <td class="font-semibold">{{ $broadcastMode->row_number }}</td>

                                    <td>{{ $broadcastMode->name }}</td>
                                    <td class="text-right">
                                        <form action="{{ route('broadcast-modes.destroy', $broadcastMode->id) }}"
                                            method="POST">
                                            @can('broadcast-modes.edit')
                                                <button type="button"
                                                    class="btn btn-ghost mask mask-squircle text-yellow-600 hover:bg-yellow-600 hover:text-white">
                                                    <a href="{{ route('broadcast-modes.edit', $broadcastMode->id) }}">
                                                        <i class="fa-regular fa-pen-to-square fa-xl"></i>
                                                    </a>
                                                </button>
                                            @endcan
                                            @can('broadcast-modes.destroy')
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
