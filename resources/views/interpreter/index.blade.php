                    ...
                    <div class="flex-none justify-end items-center">
                        @can('interpreters.create')
                            ...
                            <button type="button"
                                ...
                                <a href="{{ route('interpreters.create') }}" data-placement="left">
                    ...
                    <table id="params-table" class="table table-xs table-pin-rows table-auto">
                        ...
                        <tbody>
                            @foreach ($interpreters as $interpreter)
                                <tr class="hover">
                                    <td>{{ $interpreter->row_number }}</td>

                                    <td>{{ $interpreter->name }}</td>
                                    <td class="text-right">
                                        <form action="{{ route('interpreters.destroy', $interpreter->id) }}" method="POST">
                                            @can('interpreters.edit')
                                                <button type="button"
                                                    class="btn btn-ghost mask mask-squircle text-yellow-600 hover:bg-yellow-600 hover:text-white">
                                                    <a href="{{ route('interpreters.edit', $interpreter->id) }}">
                                                        <i class="fa-regular fa-pen-to-square fa-xl"></i>
                                                    </a>
                                                </button>
                                            @endcan
                                            @can('interpreters.destroy')
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
